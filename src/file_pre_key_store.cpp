 ...
 * Exception
 */
PreKeyStoreException::PreKeyStoreException(QString msg) : TSException(msg){
}
...
QString FilePreKeyStore::getKeyFile(qulonglong pre_key_id) {
    QString keyFileName(this->preKeyDirPath + QString("/") + QString::number(pre_key_id));

    return keyFileName;
}

void FilePreKeyStore::verifyPreKeyDir() {
...
void FilePreKeyStore::verifySignedPreKeyDir() {
...
void FilePreKeyStore::verifyPreKeys() {
...
void FilePreKeyStore::verifyLastResortPreKey() {
...
PreKeyRecord FilePreKeyStore::loadPreKey(qulonglong preKeyId) {
...
void FilePreKeyStore::storePreKey(qulonglong preKeyId, const PreKeyRecord &record) {
    QFile keyFile(this->getKeyFile(preKeyId));
...
bool FilePreKeyStore::containsPreKey(qulonglong preKeyId) {
    return QFile(this->getKeyFile(preKeyId)).exists();
}

void FilePreKeyStore::removePreKey(qulonglong preKeyId) {
    QFile keyFile(this->getKeyFile(preKeyId));
...
int FilePreKeyStore::countPreKeys() {
    QDir dir(this->preKeyDirPath + QString("/"));
    return dir.entryList().size();
}

void FilePreKeyStore::setNextPreKeyId(qulonglong id) {
...
qulonglong FilePreKeyStore::getNextPreKeyId() {
    QFile preKeyIndexFile(this->preKeyDirPath + QString("/") + "index.dat");
